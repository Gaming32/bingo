...
public class MixinBoneMealItem {
    @WrapOperation(method = "applyBonemeal", remap = false, at = @At(value = "INVOKE", target = "Lnet/minecraft/world/level/block/BonemealableBlock;performBonemeal(Lnet/minecraft/server/level/ServerLevel;Lnet/minecraft/util/RandomSource;Lnet/minecraft/core/BlockPos;Lnet/minecraft/world/level/block/state/BlockState;)V", remap = true))
    private static void wrapApplyBoneMeal(ServerLevel level, RandomSource random, BlockPos pos, BlockState state, Operation<Void> operation, ItemStack boneMeal, Level level1, BlockPos pos1, Player player) {
        GlobalVars.pushCurrentPlayer(player instanceof FakePlayer ? null : player);
        try {
            operation.call(level, random, pos, state);
        } finally {
